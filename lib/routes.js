  ...
  app.post('/api/ai-summarize', async (req, res) => {
    try {
      ...
      if (!transcriptResponse.ok) {
        ...
        console.error('Transcript service error:', transcriptResponse.status)
        throw new Error(
      ...
      if (!summaryResponse.ok) {
        ...
        console.error(
          'OpenAI API error:',
          errorData.error?.type || 'Unknown error'
        )
        throw new Error(
      ...
      // Send error response without exposing API keys
      ...
      res.status(500).json({
        ...
        service: error.message.includes('Transcript')
          ? 'transcript'
