  ...
  app.post('/api/ai-summarize', async (req, res) => {
    try {
      ...
      if (!transcriptResponse.ok) {
        ...
        throw new Error(
          `Transcript service failed: ${transcriptResponse.status} - ${errorText}`
        )
      }
      ...
      // In routes.js, update the OpenAI call:
      const summaryResponse = await fetch(
        'https://api.openai.com/v1/chat/completions',
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${process.env.OPENAI_API_KEY.trim()}`, // Add .trim()
      ...
      res.status(500).json({
        ...
        service: error.message.includes('Transcript') ? 'transcript' : 'openai'
      })
    }
  })
